...
for STEPNUMBER in $(seq 0 $(expr ${NUM_STEPS} - 1) ) ; do
	echo "srun --exclusive -n 1 -c ${THREADS_PER_STEP} -o ${LOG_FILE_DIRECTORY}/${JOB_NAME}_${GENERATION}_${STEPNUMBER}.log ${STEP_WORKER_SCRIPT} ${GENERATION} ${STEPNUMBER} &" >> ${JOB_FILE_DIRECTORY}/${JOB_NAME}_${GENERATION}.sh
done
#End Step Loop#
